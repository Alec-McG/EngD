...
fs = 200;                   % 200 Hz speed sampling
fs_enc = 10;                % 10 Hz encoder sampling
dtEnc = 1/fs_enc;           % Encoder timestep
encVar = 0.5;               % Encoder Variance
sim_length = 3000;          % Straight line length
t_len = sim_length/v;       % Time moving along raster length

gyroThetaVar = 0.2*pi/180;         % Variance of Gyroscope measurment
...
for i=1:length(pose_noise_v)
    thetaGyroMeas(i) = Theta0 + rand(1)*gyroThetaVar; 
